...
impl PayloadContentConnect {
    ...
    pub fn into_bytes(&self) -> Vec<u8> {
        ...
        payload_bytes.extend(self.client_id.into_bytes());

        if let Some((will_topic, will_message)) = &self.will {
            payload_bytes.extend(will_topic.into_bytes());
            payload_bytes.extend(will_message.into_bytes());
        }

        if let Some((username, password)) = &self.user {
            payload_bytes.extend(username.into_bytes());

            if let Some(password) = password {
                payload_bytes.extend(password.into_bytes());
            }
        }
