...
use sauron::model::{
    ...
    components::qos::QoS,
    components::topic_level::TopicLevel,
    components::topic_name::TopicName,
};
...
impl Client {
    ...
    pub fn publish(&self, topic: &str, message: &str) -> std::io::Result<()> {
        ...
        let _ = self.to_server_stream.lock().unwrap().as_mut().unwrap().write(publish_packet.to_bytes().as_slice());
